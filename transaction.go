...
func CreateSignedTransaction(ctx context.Context, ops []*protocol.Operation, key *KoinosKey, nonce uint64, rcLimit uint64, chainID []byte, payer []byte) (*protocol.Transaction, error) {
...
func CreateTransaction(ctx context.Context, ops []*protocol.Operation, address []byte, nonce uint64, rcLimit uint64, chainID []byte, payer []byte) (*protocol.Transaction, error) {
	...
	nonceBytes, err := UInt64ToNonceBytes(nonce)
	if err != nil {
		return nil, err
	}
	...
	// Find merkle root
	merkleRoot, err := CalculateMerkleRoot(opHashes)
	if err != nil {
		return nil, err
	}
